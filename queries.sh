...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games 
      ON games.winner_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014
      UNION
      SELECT name FROM teams FULL JOIN games 
      ON games.opponent_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014
      ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo  "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games ON winner_id = team_id WHERE year IS NOT NULL ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM teams FULL JOIN games ON winner_id = team_id WHERE round = 'Final' ORDER BY year")"
